...
public final class EmailNotificationService {
  ...
  public void notifyUserCreated(final UserModel user, final String plainPassword) {
    // Clean Code - Regla 25 (claridad sobre ingenio) y Regla 26 (evitar sobre-compactación):
    ...
    // es idéntica a la de notifyUserUpdated — debería centralizarse.
      ...
      // Solución:
      // Separar las operaciones en variables intermedias y extraer la
      // construcción de tokens a métodos reutilizables.

      final String template = loadTemplate("user-created.html");

      final Map<String, String> tokens =
              buildCreatedUserTokens(user, plainPassword);
  ...
  public void notifyUserUpdated(final UserModel user) {
    ...
    // Clean Code - Regla 25 y 26: misma sobre-compactación que arriba.

      final String template = loadTemplate("user-updated.html");

      final Map<String, String> tokens =
              buildUpdatedUserTokens(user);
  ...
  // y en el fallido loguea Y relanza la excepción.
  ...
  private void sendEmail(final EmailDestinationModel destination) {
    ...
    private Map<String, String> buildCreatedUserTokens(
            final UserModel user,
    ...
    private Map<String, String> buildUpdatedUserTokens(
            final UserModel user) {
